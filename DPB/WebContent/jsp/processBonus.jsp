...
document.myform.action ='<%=request.getContextPath()%>' +'/viewProcBonusProcpopup.action?procDpbID='+val;
...
function fun(pId,procType,pType)
{
	...
	document.getElementById('myform').action = "bonusProcessing";
...
<div id="HL1"><span class="pageTitle">DPB Bonus Processing</span></div>
	...
	<div class="T8">
		<table cellspacing="0" class="template8TableTop" border="0">
			...
			<tr>
				<td id="ctl00_tdCopyText" class="left"><div class="TX1">Bonus Process Details are displayed here. The manual bonus process can be triggered here.</div></td>
...
<s:form name="myform" id="bonusProcessing" action="bonusProcessing">
		...
		<td width="1417"><table width="949"  border="0" class="placeholderTable">
		  <tr>
		    ...
		    <td width="100"><strong>Action</strong></td>
					...
					<tr>
					<s:if test="%{processBonusList.isEmpty()}"></s:if>
					<s:else>
					<s:iterator value="processBonusList">
					<tr>
					...
					<td ><s:submit action="bonusProcessingStartProc" theme="simple" key="submit" value="Start Process" align="left" onclick="fun('%{pID}','%{pType}','');"/>&nbsp;
					</td>
					</s:elseif>
					 <s:if test='%{userActionRole == true}'>
						<s:if test='reprocess.trim().equals("P")'>
							<s:set name="processType" value=""/>
							<td ><s:submit  theme="simple"  key="submit"  action="bonusProcessingStartProc" value="Re-Process" align="left"  onclick="fun('%{pID}','%{pType}','');" />&nbsp;
							</td>
						</s:if>
					</s:if>
					<s:if test='%{resetAllowed == true}'>
						<s:if test='reset.trim().equals("T")'>
							<s:set name="processType" value=""/>
							<td ><s:submit  theme="simple"  key="submit"  action="bonusProcessingStartProc" value="RESET" align="left"  onclick="fun('%{pID}','%{pType}','T');" />&nbsp; 
							</td>
						</s:if>
					</s:if>
			      	 </tr>
