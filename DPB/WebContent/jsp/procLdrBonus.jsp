...
<html>
<head>
<title>Leadership Bonus Processing</title>
...
document.myform.action ='<%=request.getContextPath()%>' +'/viewLdrBonuspopup.action?procDpbID='+val;
//document.getElementById("submit").submit();
openPopup(document.myform.action,'fixed');
}

function fun(pId,procType,pType)
{
	...
	document.getElementById("processType").value = pType;
	//alert(procDpbID);
	//alert(document.forms[0].name);
	//$("#fileProcessingInp").submit();
	/* document.forms[0].action = 'fileProcessingInp';*/
	document.getElementById('myform').action = "processLdrBonus";
...
<div id="HL1"><span class="pageTitle">DPB Leadership Bonus Processing</span></div>
	...
	<div class="T8">
		<table cellspacing="0" class="template8TableTop" border="0">
			...
			<tr>
				<td id="ctl00_tdCopyText" class="left"><div class="TX1">Leadership Bonus Process Details are displayed here. The manual leadership bonus process can be triggered here.</div></td>
...
<s:form name="myform" id="processLdrBonus" action="processLdrBonus">
...
<table width="148%" cellpadding="0" cellspacing="0" class="TBL2">
	<tr>
		<td width="1417"><table width="949"  border="0" class="placeholderTable">
		  ...
		  <tr>
			<s:if test="%{processLdrBonusList.isEmpty()}">	
			</s:if>
			<s:else>
				  <s:iterator value="processLdrBonusList">
				  	<tr>
					   ...
					   <s:elseif test='flag.trim().equals("S")'>
					  	<td ><s:submit action="processLdrBonusStartProc" theme="simple" key="submit" value="Start Process" align="left" onclick="fun('%{pID}','%{pType}');"/>&nbsp;
					  		</td>
					   </s:elseif>
					   <s:if test='%{userActionRole == true}'>
						    <s:if test='reprocess.trim().equals("P")'>
						   <s:set name="processType" value=""/>
						  	<td ><s:submit  theme="simple"  key="submit"  action="processLdrBonusStartProc" value="Re-Process" align="left"  onclick="fun('%{pID}','%{pType}','');" />&nbsp;
						  	</td>
						  </s:if>
					   </s:if>
					   <s:if test='%{resetAllowed == true}'>
						   <s:if test='reset.trim().equals("T")'>
						   <s:set name="processType" value=""/>
						  	<td ><s:submit  theme="simple"  key="submit"  action="processLdrBonusStartProc" value="RESET" align="left"  onclick="fun('%{pID}','%{pType}','T');" />&nbsp; 
						  	</td>
						  </s:if>
						</s:if>
			      	 </tr>
