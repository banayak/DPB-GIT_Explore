...
document.myform.action ='<%=request.getContextPath()%>' +'/viewGenPaypopup.action?procDpbID='+val;
//document.getElementById("submit").submit();
openPopup(document.myform.action,'fixed');
}

function fun(pId,procType)
{
	...
	document.getElementById('myform').action = "genPayFile";
...
<div id="HL1"><span class="pageTitle">DPB Payment Processing</span></div>
	...
	<div class="T8">
		<table cellspacing="0" class="template8TableTop" border="0">
			...
			<tr>
				<td id="ctl00_tdCopyText" class="left"><div class="TX1">Payment Process Details are displayed here. The manual Payment process can be triggered here.</div></td>
...
<s:form name="myform" id="genPayFile" action="genPayFile">
...
<table width="148%" cellpadding="0" cellspacing="0" class="TBL2">
	<tr>
		<td width="1417"><table width="949"  border="0" class="placeholderTable">
		  ...
		  <tr>
			<s:if test="%{genPaymentFileList.isEmpty()}">	
			</s:if>
			<s:else>
				  <s:iterator value="genPaymentFileList">
				  	<tr>
					   ...
					   <s:elseif test='flag.trim().equals("S")'>
					  	<td ><s:submit action="genPayFileStartProc" theme="simple" key="submit" value="Start Process" align="left" onclick="fun('%{pID}','%{pType}');"/>&nbsp;
					  		</td>
					   </s:elseif>
			      	 </tr>
			      </s:iterator>
