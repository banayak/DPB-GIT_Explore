...
String newQuery = WebHelper.makeNonNullString(queryStr);
			try {
			if(!newQuery.isEmpty()){
				if(!newQuery.toUpperCase().contains("SELECT")){
				...
				if(!newQuery.toUpperCase().contains("UPDATE")){ 
					...
					st.execute();
					%>
			<H1>Record(s) Deleted Successfully</H1>
			<%
}
}
			...
			if(newQuery.toUpperCase().contains("SELECT") || newQuery.toUpperCase().contains("INSERT") || (newQuery.toUpperCase().contains("UPDATE"))){
			%>
			<H1>No select/insert/update operation is allowed</H1>
			<%
			}
