...
public static final String LEADERSHIP_SUMMARY_REPORT_CARS = "SELECT ID_DLR , count(NUM_VIN)  FROM DPBUSER.DPB_UNBLK_VEH AS CALC" +
		"	WHERE DTE_RTL BETWEEN 'YR_START_DT' AND 'YR_END_DT' AND CDE_VEH_TYP IN ('P') " +
		"AND IND_USED_VEH_DDRS = 'N' AND CDE_VEH_DDR_STS = 'I1' AND CHAR(DTE_RTL) || CHAR(TME_RTL) = (SELECT MAX(CHAR(B.DTE_RTL) || CHAR(B.TME_RTL)) " +
		" FROM DPBUSER.DPB_UNBLK_VEH AS B  WHERE CALC.NUM_VIN = B.NUM_VIN  AND B.DTE_RTL BETWEEN 'YR_START_DT' AND 'YR_END_DT')";
